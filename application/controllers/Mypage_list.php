...
class Mypage_list extends CI_Controller
{
    public function index()
    {
        if ((int)$this->session->authority === 0) {
            // config data取得
            ...
            $data['mypage_title'] = '従業員 勤務状況（集計）';

            // 表示用データ
            $data['page_id'] = 'mypage_list';
            $data['page_title'] = $this->session->user_name;
            $data['company_name'] = $this->session->user_name;

            // view
            $this->load->view('mypage/list_view', $data);
        } else {
            redirect('/');
        }
    }
