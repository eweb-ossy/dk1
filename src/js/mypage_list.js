...
(function() {
  ...
  if (Cookies.get('MypagetoListsuserID')) {
    var user_id = Cookies.get('MypagetoListsuserID');
    var date = new Date(Cookies.get('MypagetoListsMonth'));
  } else {
  ...
  var view = {
    ...
    renderTableColumns: new Promise(function(resolve, reject) {
      model.getTableColumns().done(function(data) {
        ...
        if (mypage_status_inout_view_flag === 1) {
          table_list_user_detail.hideColumn('in_work_time');
          table_list_user_detail.hideColumn('out_work_time');
        }
        if (mypage_status_inout_view_flag === 2) {
          table_list_user_detail.hideColumn('in_time');
          table_list_user_detail.hideColumn('out_time');
        }
      });
    }),
    // view テーブルデータ表示
    ...
    render_modal: function(row_data) {
      ...
      //
      $('#time_edit_submit').attr('data-user-id', user_id);
