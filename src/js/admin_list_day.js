(function() {
    ...
    function formatDate(date, format) { // 指定の日付フォーマットを返す
    ...
    const model = {
        ...
        listData: function(list_data) {
            // 総労働時間
            let all_work_data = list_data.filter( element => element.work_hour2 > 0 ); // 勤務時間 > 0 の従業員データを抽出
            ...
            // 総シフト時間
            let all_shift_work_data = list_data.filter( element => element.shift_hour2 > 0 );
            all_data['shift_hour2'] = all_shift_work_data.reduce( (result, current) => {
                return result + Number(current.shift_hour2)
            }, 0);
            all_data['shift_hour'] = all_data['shift_hour2'] > 0 ? minuteToStr(all_data['shift_hour2']) : '';

            // 総休憩時間
            let all_rest_data = list_data.filter( element => element.rest_hour2 > 0 );
            all_data['rest_minute'] = all_rest_data.reduce( (result, current) => {
                return result + Number(current.rest_hour2)
            }, 0);
            all_data['rest_hour'] = all_data['rest_minute'] > 0 ? minuteToStr(all_data['rest_minute']) : '';
    ...
    const view = {
            ...
            model.getTableColumns().done( data => {
                ...
                download_column.forEach( (element, index) => {
                    ...
                    if (!element.field) { // fieldが無い = グループコラムの時
                        element.columns.forEach( val => { // column groupを書き出す
        ...
        renderTableData: function() {
            ...
            let date_format = formatDate(new Date(date), 'YYYY-MM-DD');
            let now_date = formatDate(new Date(), 'YYYY-MM-DD');
            if (date_format === now_date) {
                ...
                let diff_day = Math.ceil((date - new Date()) / 86400000);
                if (diff_day > 0) {
            ...
            let wareki = new Intl.DateTimeFormat('ja-JP-u-ca-japanese', {era: 'long', year: 'numeric'}).format(new Date(date)); // 和暦
            $('#date-area-wareki').text(wareki);

            // テーブル表示
            table_list_day.clearData();
            table_list_day.replaceData('../data/admin_list_day/table_data', { date: date_format }, 'POST')
            .then(function() {
                view.renderListData();
            })
        },
    ...
    $(function() {
        ...
        function changeDate(diff) {
            date = diff ? new Date(date.setDate(date.getDate() + diff)) : new Date();
            view.renderTableData();
        }
        $('#less_day').on('click', function() { // 戻るボタン
            changeDate(-1);
        });
        $('#add_day').on('click', function() { // 次へボタン
            changeDate(1);
        });
        $('#today_mark, #date_today').on('click', function() { // 本日ボタン
            changeDate();
        });
        // 詳細表示ボタン　クリック
